...
\begin{block}{Limitação do Bagging}
\phantomsection\label{limitauxe7uxe3o-do-bagging}
Mesmo com bootstrap, as árvores podem ser \textbf{correlacionadas}:
...
\begin{frame}{Ideia central do algoritmo}
\phantomsection\label{ideia-central-do-algoritmo}
Cada peso é atualizado usando:

\[
\text{erro} \times \text{sensibilidade local}
\]
\end{frame}
...
\tightlist
\item
  erro → o quanto erramos\\
\item
  derivada → como corrigir
\end{itemize}
\end{block}
\end{frame}
...
Backpropagation = aplicação eficiente da regra da cadeia
\end{block}
\end{frame}

\begin{frame}{Derivada do erro (exemplo)}
\phantomsection\label{derivada-do-erro-exemplo}
Para a camada de saída:

\[
\frac{\partial EQM(g_\beta)}{\partial \beta_{j,1}^{(H)}}
=
\frac{2}{n} \sum_{k=1}^{n}
\left(
g_\beta(x_k) - y_k
...
\item
  erro × entrada\\
\item
  exatamente como regressão linear
\end{itemize}
\end{block}
\end{frame}
